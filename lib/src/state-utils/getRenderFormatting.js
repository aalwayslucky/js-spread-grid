    ...
    return [
        ...
        }),
        {
            ...
            style: ({ rows, columns, row, column, edit }) => ({
                ...(!isSelected(rows, columns, row.index - 1, column.index) ? { borderTop: { width: 1, color: '#31A7E380', index: Number.MAX_SAFE_INTEGER } } : {}),
                ...(!isSelected(rows, columns, row.index + 1, column.index) ? { borderBottom: { width: 1, color: '#31A7E380', index: Number.MAX_SAFE_INTEGER } } : {}),
                ...(!isSelected(rows, columns, row.index, column.index - 1) ? { borderLeft: { width: 1, color: '#31A7E380', index: Number.MAX_SAFE_INTEGER } } : {}),
                ...(!isSelected(rows, columns, row.index, column.index + 1) ? { borderRight: { width: 1, color: '#31A7E380', index: Number.MAX_SAFE_INTEGER } } : {}),
                highlight: getHighlightColor(edit ? '#31A7E3' : '#041822', focusedColumnKey !== column.key || focusedRowKey !== row.key)
            }),
        },
        {
            column: { type: 'ANY' },
